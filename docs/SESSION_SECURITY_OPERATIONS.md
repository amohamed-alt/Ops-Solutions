...
Remove `--dry-run` only after reviewing the candidate count. The query is scoped to the supplied user UUID.
...
Execution keeps the most recently active sessions and revokes only overflow sessions for that user.
...
- Destructive library operations default to dry-run.
