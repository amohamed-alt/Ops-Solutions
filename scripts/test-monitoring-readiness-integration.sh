...
for file in "$runner" "$installer" "$status" "$notification_wrapper"; do
  [[ -f "$file" ]] || { echo "Missing $file" >&2; exit 1; }
  bash -n "$file"
done

grep -Fq 'backup|sla|integrity|readiness|readiness-incidents|readiness-notifications' "$runner"
...
grep -Fq "('backup', 'sla', 'readiness', 'readiness-incidents', 'readiness-notifications', 'integrity')" "$status"
