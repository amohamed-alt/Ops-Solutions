...
function printText(result) {
  if (result.summary) {
    ...
    console.log(`Stale sessions: ${result.summary.stale_sessions ?? 0}`);
    console.log(`Users above cap: ${result.riskyUsers.length}`);
    for (const row of result.riskyUsers) {
      console.log(`- ${row.email}: ${row.active_session_count} active sessions`);
    }
    return;
  }
...
try {
  ...
  if (options.action === 'status') {
    result = await inspectSessionSecurity(postgres, options);
  } else if (options.action === 'prune-expired') {
    result = await pruneExpiredSessions(postgres, { dryRun: options.dryRun });
  } else if (options.action === 'revoke-user') {
    result = await revokeUserSessions(postgres, options.userId, { dryRun: options.dryRun });
  } else if (options.action === 'enforce-cap') {
    result = await enforceSessionCap(postgres, options.userId, options.maxActiveSessions, { dryRun: options.dryRun });
  } else {
    throw new Error('Action must be status, prune-expired, revoke-user, or enforce-cap.');
  }
