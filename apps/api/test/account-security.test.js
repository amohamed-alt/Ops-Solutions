...
test('serializes sessions without exposing token or IP hashes', () => {
  ...
  assert.equal(value.explicitlyTrusted, true);
  assert.equal(value.risk.level, 'trusted');
  assert.equal(value.risk.reason, 'Current session on a trusted device');
...
test('classifies unfamiliar, trusted, dormant and aged sessions deterministically', () => {
  assert.deepEqual(classifySessionRisk({
    ...
    level: 'review', reason: 'New device or browser', dormantDays: 0, familiarDevice: false, explicitlyTrusted: false
  });
  assert.equal(classifySessionRisk({
    current_session: true,
    known_device: true,
    explicitly_trusted: true,
    created_at: '2026-07-23T00:00:00Z',
    last_seen_at: '2026-07-24T00:00:00Z'
  }, NOW).reason, 'Current session on a trusted device');
  assert.deepEqual(classifySessionRisk({
    ...
    level: 'high', reason: 'Inactive for 34 days', dormantDays: 34, familiarDevice: true, explicitlyTrusted: false
  });
...
test('account security schema is idempotent, extensible and replica safe', async () => {
  ...
  assert.match(sql, /POSITION\('device\.trusted'/);
...
test('registers protected list, trust and revocation routes with account-scoped SQL', async () => {
  const routes = [];
  const queries = [];
  const postgres = {
    async query(text, values = []) {
      ...
      if (text.includes('INSERT INTO account_trusted_devices')) {
        return { rowCount: 1, rows: [{ id: '22222222-2222-4222-8222-222222222222', trusted_at: NOW.toISOString() }] };
      }
  ...
  await routes[0].options.preHandler(request, reply);
  ...
  assert.match(sessionQuery.text, /account_trusted_devices/);
  assert.match(sessionQuery.text, /fingerprint_hash/);
  assert.match(sessionQuery.text, /digest\(COALESCE\(s\.user_agent/);
  ...
  assert.match(sessionQuery.text, /AS explicitly_trusted/);
  ...
  await trustRoute.options.preHandler(request, reply);
  const trustPayload = await trustRoute.handler(request, reply);
  assert.equal(reply.statusCode, 201);
  assert.equal(trustPayload.trusted, true);
  const trustQuery = queries.find((entry) => entry.text.includes('INSERT INTO account_trusted_devices'));
