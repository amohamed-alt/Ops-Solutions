...
const SOURCE_URL = new URL('../src/webhook-recovery-cli.js', import.meta.url);

test('parses bounded workspace-scoped recovery arguments', () => {
  assert.deepEqual(parseWebhookRecoveryArguments([
    '--action', 'retry', '--workspace', WORKSPACE_ID,
    '--event', EVENT_ID, '--event', EVENT_ID,
    '--limit', '25', '--dry-run'
  ]), {
    action: 'retry', workspaceId: WORKSPACE_ID, status: '', limit: 25,
    eventIds: [EVENT_ID], dryRun: true, staleHours: 24, onlyUnhealthy: false
  });
});

test('parses read-only fleet health arguments without a workspace', () => {
  assert.deepEqual(parseWebhookRecoveryArguments([
    '--action', 'fleet', '--only-unhealthy', '--stale-hours', '12'
  ]), {
    action: 'fleet', workspaceId: '', status: '', limit: 50,
    eventIds: [], dryRun: false, staleHours: 12, onlyUnhealthy: true
  });
});
...
test('classifies fleet workspace health by production priority', () => {
  ...
  const base = {
    workspace_id: WORKSPACE_ID, workspace_name: 'Acme', portal_id: '123',
    hubspot_status: 'connected', total: 10, failed: 0, pending: 0,
    queued: 10, ignored: 0, latest_received_at: '2026-07-23T00:30:00Z',
    latest_processed_at: '2026-07-23T00:31:00Z', latest_sync_at: '2026-07-23T00:40:00Z'
  };
...
test('aggregates webhook and CRM health independently to avoid portal-size multiplication', async () => {
  const source = await readFile(SOURCE_URL, 'utf8');
  assert.match(source, /WITH event_health AS/);
  assert.match(source, /sync_health AS/);
  assert.doesNotMatch(source, /LEFT JOIN hubspot_webhook_events e[\s\S]*LEFT JOIN crm_records r/);
});
