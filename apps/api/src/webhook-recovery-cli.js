...
export function parseWebhookRecoveryArguments(argv) {
  ...
  const options = {
    action: 'status', workspaceId: '', status: '', limit: 50,
    eventIds: [], dryRun: false, staleHours: 24, onlyUnhealthy: false
  };
  ...
  if (options.action !== 'fleet' && !UUID_PATTERN.test(options.workspaceId)) throw new Error('A valid --workspace UUID is required.');
  if (options.action === 'fleet' && options.workspaceId) throw new Error('Fleet health does not accept --workspace. Use status for one workspace.');
  ...
  if (!Number.isFinite(options.staleHours) || options.staleHours < 1 || options.staleHours > 720) throw new Error('--stale-hours must be between 1 and 720.');
  options.eventIds = [...new Set(options.eventIds.filter(Boolean))];
  if (options.action === 'ignore' && options.eventIds.length === 0) throw new Error('Ignore requires at least one --event UUID.');
  return options;
}

export function recoveryModeForEvents(events) {
  return events.some((event) => event.action === 'association_changed' || event.action === 'deleted') ? 'full' : 'incremental';
}
...
export function classifyFleetWorkspace(row, { now = Date.now(), staleHours = 24 } = {}) {
  ...
  if (connectionStatus !== 'connected') [health, reason] = ['disconnected', 'HubSpot OAuth is not connected.'];
  else if (failed > 0) [health, reason] = ['degraded', `${failed} webhook event${failed === 1 ? '' : 's'} require recovery.`];
  else if (pending > 0) [health, reason] = ['pending', `${pending} webhook event${pending === 1 ? '' : 's'} are waiting to be queued.`];
  else if (latestSyncAt && now - latestSyncAt > staleAfterMs) [health, reason] = ['stale', `The CRM mirror has not synchronized within ${staleHours} hours.`];
  else if (!latestReceivedAt) [health, reason] = ['no_webhooks', 'No webhook delivery has been observed for this workspace.'];
  return {
    workspaceId: String(row.workspace_id), workspaceName: String(row.workspace_name),
    portalId: row.portal_id ? Number(row.portal_id) : null, connectionStatus, health, reason,
    counts: { total: Number(row.total ?? 0), failed, pending, queued: Number(row.queued ?? 0), ignored: Number(row.ignored ?? 0) },
...
async function requireWorkspace(workspaceId) {
  const result = await postgres.query(
    `SELECT w.id, w.name, c.status AS hubspot_status FROM workspaces w
     LEFT JOIN hubspot_connections c ON c.workspace_id = w.id WHERE w.id = $1 LIMIT 1`, [workspaceId]
  );
...
async function fleetHealth({ staleHours, onlyUnhealthy }) {
  const result = await postgres.query(`
    WITH event_health AS (
      ...
      SELECT workspace_id, MAX(synced_at) AS latest_sync_at
      FROM crm_records GROUP BY workspace_id
    )
    SELECT w.id AS workspace_id, w.name AS workspace_name, c.portal_id,
           c.status AS hubspot_status,
           COALESCE(e.total, 0) AS total, COALESCE(e.failed, 0) AS failed,
           COALESCE(e.pending, 0) AS pending, COALESCE(e.queued, 0) AS queued,
           COALESCE(e.ignored, 0) AS ignored, e.latest_received_at,
           e.latest_processed_at, s.latest_sync_at
    FROM workspaces w
    LEFT JOIN hubspot_connections c ON c.workspace_id = w.id
    LEFT JOIN event_health e ON e.workspace_id = w.id
    LEFT JOIN sync_health s ON s.workspace_id = w.id
...
async function status(workspaceId) {
  const result = await postgres.query(
    `SELECT COUNT(*)::int AS total,
            ...
            COUNT(*) FILTER (WHERE status = 'queued')::int AS queued,
            COUNT(*) FILTER (WHERE status = 'ignored')::int AS ignored,
            MAX(received_at) AS latest_received_at, MAX(processed_at) AS latest_processed_at
     FROM hubspot_webhook_events WHERE workspace_id = $1`, [workspaceId]
  );
  return result.rows[0];
}
...
async function listEvents({ workspaceId, status: selectedStatus, limit }) {
  ...
  const result = await postgres.query(
    `SELECT id, subscription_type, object_type, object_id, action, property_name,
     ...
     FROM hubspot_webhook_events WHERE workspace_id = $1 ${filter}
     ORDER BY received_at DESC LIMIT $2`, values
  );
  return result.rows;
}
...
async function selectRetryableEvents({ workspaceId, eventIds, limit }) {
  if (eventIds.length > 0) {
    return (await postgres.query(
      `SELECT id, action, object_type, object_id, status FROM hubspot_webhook_events
       WHERE workspace_id = $1 AND id = ANY($2::uuid[]) AND status IN ('failed', 'received')
       ORDER BY received_at ASC`, [workspaceId, eventIds]
    )).rows;
  }
  return (await postgres.query(
    `SELECT id, action, object_type, object_id, status FROM hubspot_webhook_events
     ...
     ORDER BY received_at ASC LIMIT $2`, [workspaceId, limit]
  )).rows;
}
...
async function retryEvents(options, workspace) {
  ...
  const queue = new Queue('hubspot-sync', {
    ...
    defaultJobOptions: { attempts: 4, backoff: { type: 'exponential', delay: 30_000 }, removeOnComplete: { age: 86_400, count: 1000 }, removeOnFail: { age: 604_800, count: 1000 } }
  });
  try {
    const jobName = jobNameForMode(mode);
    const job = await queue.add(jobName, {
      workspaceId: options.workspaceId, requestedAt: new Date().toISOString(),
      requestedBy: 'webhook_recovery_cli', source: 'webhook_recovery_cli',
      webhookEventIds: events.map((event) => event.id), eventCount: events.length
    }, { jobId: `webhook-recovery-${jobName}-${options.workspaceId.replaceAll('-', '')}-${Date.now()}` });
    await postgres.query(
      `UPDATE hubspot_webhook_events SET status = 'queued', error = NULL,
...
async function ignoreEvents(options) {
  ...
  const result = await postgres.query(
    `UPDATE hubspot_webhook_events SET status = 'ignored', error = NULL,
     processed_at = NOW(), updated_at = NOW()
     WHERE workspace_id = $1 AND id = ANY($2::uuid[]) AND status IN ('failed', 'received')
     RETURNING id, subscription_type, object_type, object_id`, [options.workspaceId, options.eventIds]
  );
...
async function main() {
  ...
  console.log(JSON.stringify(await runWebhookRecovery(options), null, 2));
}

if (process.argv[1] && import.meta.url === new URL(`file://${process.argv[1]}`).href) {
  main().catch((error) => { console.error(JSON.stringify({ error: error.message }, null, 2)); process.exitCode = 1; }).finally(() => postgres.end());
}
