...
const DATE_PRESETS = new Set(['today','yesterday','last_7_days','last_30_days','this_month','previous_month','this_quarter','this_year','custom']);
...
export function normalizeSavedView(input = {}) {
  ...
  const filters = Object.fromEntries(FILTER_KEYS.map((key) => [key, text(input.filters?.[key])]));
  ...
  return {
    ...
    datePreset,
    filters,
    section: SECTIONS.has(input.section) ? input.section : 'overview',
...
export async function ensureSavedViewsSchema(postgres) {
  await postgres.query(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS saved_reporting_views_one_default_idx
      ON saved_reporting_views(workspace_id, user_id) WHERE is_default = TRUE;
  `);
}
...
async function audit(postgres, request, action, targetId, metadata) {
  await postgres.query(
...
export function registerSavedViewRoutes(app, { postgres }) {
  ...
  async function requireAccess(request, reply) {
    await schemaReady;
    ...
    if (result.rowCount === 0) return reply.code(403).send({ error: 'workspace_forbidden', message: 'This workspace is not available to your account.' });
    request.savedViewAccess = { userId: result.rows[0].user_id, workspaceId: result.rows[0].workspace_id };
  }

  app.get('/api/v1/customer/workspaces/:workspaceId/saved-views', { preHandler: requireAccess }, async (request) => {
    const result = await postgres.query(
      `SELECT * FROM saved_reporting_views WHERE workspace_id = $1 AND user_id = $2
  ...
  app.post('/api/v1/customer/workspaces/:workspaceId/saved-views', { preHandler: requireAccess }, async (request, reply) => {
    const view = normalizeSavedView(request.body);
    try {
      const result = await postgres.query(
        `WITH cleared AS (
           UPDATE saved_reporting_views SET is_default = FALSE, updated_at = NOW()
           WHERE $7::boolean AND workspace_id = $1 AND user_id = $2 AND is_default = TRUE
         )
      ...
      await audit(postgres, request, 'reporting_view.created', result.rows[0].id, { name: view.name, isDefault: view.isDefault });
  ...
  app.patch('/api/v1/customer/workspaces/:workspaceId/saved-views/:viewId', { preHandler: requireAccess }, async (request, reply) => {
    const view = normalizeSavedView(request.body);
    try {
      const result = await postgres.query(
        `WITH cleared AS (
           UPDATE saved_reporting_views SET is_default = FALSE, updated_at = NOW()
           WHERE $8::boolean AND workspace_id = $1 AND user_id = $2 AND id <> $3 AND is_default = TRUE
         )
      ...
      await audit(postgres, request, 'reporting_view.updated', request.params.viewId, { name: view.name, isDefault: view.isDefault });
  ...
  app.post('/api/v1/customer/workspaces/:workspaceId/saved-views/:viewId/duplicate', { preHandler: requireAccess }, async (request, reply) => {
    ...
    try {
      const result = await postgres.query(
        `INSERT INTO saved_reporting_views(workspace_id,user_id,name,date_preset,filters,section,is_default)
         ...
         FROM saved_reporting_views WHERE workspace_id=$1 AND user_id=$2 AND id=$3 RETURNING *`,
        [request.savedViewAccess.workspaceId, request.savedViewAccess.userId, request.params.viewId, name]
      );
      if (result.rowCount === 0) return reply.code(404).send({ error: 'saved_view_not_found', message: 'Saved view not found.' });
      await audit(postgres, request, 'reporting_view.duplicated', result.rows[0].id, { name, sourceId: request.params.viewId });
      return reply.code(201).send(serialize(result.rows[0]));
    } catch (error) {
      if (error.code === '23505') return reply.code(409).send({ error: 'saved_view_exists', message: 'A saved view with this name already exists.' });
      throw error;
  ...
  app.delete('/api/v1/customer/workspaces/:workspaceId/saved-views/:viewId', { preHandler: requireAccess }, async (request, reply) => {
    const result = await postgres.query(
      `DELETE FROM saved_reporting_views WHERE workspace_id=$1 AND user_id=$2 AND id=$3 RETURNING id,name`,
      [request.savedViewAccess.workspaceId, request.savedViewAccess.userId, request.params.viewId]
    );
    if (result.rowCount === 0) return reply.code(404).send({ error: 'saved_view_not_found', message: 'Saved view not found.' });
    await audit(postgres, request, 'reporting_view.deleted', request.params.viewId, { name: result.rows[0].name });
