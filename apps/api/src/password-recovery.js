...
async function enforceLimit(redis, key, limit, ttlSeconds) {
...
export function registerPasswordRecoveryRoutes(app, {
  ...
  app.post('/api/v1/auth/password/forgot', async (request, reply) => {
    ...
    await Promise.all([
      enforceLimit(redis, `password-reset:email:${hashValue(email || 'invalid')}`, REQUEST_LIMIT_PER_EMAIL, REQUEST_WINDOW_SECONDS),
      enforceLimit(redis, `password-reset:ip:${ipHash}`, REQUEST_LIMIT_PER_IP, REQUEST_WINDOW_SECONDS)
    ]);
    ...
    if (!user) return reply.code(202).send(generic);
    ...
    await withTransaction(async (client) => {
      ...
      await client.query(
        `INSERT INTO password_reset_tokens (
           token_hash, user_id, expires_at, requested_ip_hash, requested_user_agent
         ) VALUES ($1, $2, NOW() + ($3::int * INTERVAL '1 minute'), $4, $5)`,
        [hashValue(token), user.id, TOKEN_TTL_MINUTES, ipHash, safeText(request.headers['user-agent'], 500) || null]
      );
    });
    ...
    if (emailConfig.configured) {
      ...
      try {
        await sendEmailImpl(emailConfig, {
          ...
          ...message,
          idempotencyKey: `password-reset-${hashValue(token).slice(0, 32)}`
        });
      } catch (error) {
        request.log.error({ category: error.category, retryable: error.retryable }, 'Password reset delivery failed');
      }
    } else {
      request.log.warn({ missing: emailConfig.missing }, 'Password reset email provider is not configured');
    }
  ...
  app.post('/api/v1/auth/password/reset', async (request, reply) => {
    ...
    const ipHash = hashValue(requestIp(request));
    await enforceLimit(redis, `password-reset:consume:${ipHash}`, RESET_LIMIT_PER_IP, RESET_WINDOW_SECONDS);
    ...
    const result = await withTransaction(async (client) => {
      ...
      return { userId, revokedSessions: revoked.rowCount };
    });
