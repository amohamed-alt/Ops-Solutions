...
type ObjectBreakdown = { key: string; label: string; rows: Array<{ key: string; value: number }> };
type ObjectDetail = {
  generatedAt: string;
  objectType: ObjectType;
  label: string;
...
type ObjectMeta = { label: string; shortLabel: string; icon: LucideIcon };
...
const OBJECT_TYPES = Object.keys(OBJECT_META) as ObjectType[];
...
function queryString(filters: DateRange, extra: Record<string, string | number> = {}) {
...
async function fetchJson<T>(input: RequestInfo | URL, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) throw new Error(payload.message || 'Unable to load reporting data.');
  return payload;
}
...
function hubSpotRecordUrl(portalId: string, objectType: ObjectType, recordId: string) {
  ...
  return `${base}/record/${HUBSPOT_OBJECT_TYPE_IDS[objectType]}/${encodeURIComponent(recordId)}`;
}

function recordLabel(objectType: ObjectType, row: DrilldownRow) {
...
function ObjectCard({ row, active, onSelect }: { row: ObjectOverviewRow; active: boolean; onSelect: () => void }) {
  const meta = OBJECT_META[row.objectType];
  const Icon = meta.icon;
  return (
    <button type="button" className={`oi-object-card${active ? ' active' : ''}`} onClick={onSelect} aria-pressed={active}>
...
function MetricCard({ metric, currency, onOpen }: { metric: ObjectMetric; currency: string; onOpen: () => void }) {
  return (
    <button type="button" className={`oi-metric-card oi-tone-${metric.tone}`} onClick={onOpen} title={`Open ${metric.label} records`}>
...
function ObjectDashboard({
  ...
  if (loading && !detail) {
    return <div className="oi-detail-skeleton" aria-label="Loading object reports">{Array.from({ length: 8 }, (_, index) => <span key={index} />)}</div>;
  }
  if (error && !detail) return <div className="oi-error">{error}</div>;
  if (!detail) return null;

  return (
    <>
      {error ? <div className="oi-warning">{error} Showing the last successful snapshot.</div> : null}
      <div className="oi-section-heading">
        <div><span>OBJECT REPORT PACK</span><h3>{detail.label} intelligence</h3><p>{detail.description}</p></div>
        <b>{detail.metrics.length} live reports</b>
      </div>
      <div className="oi-metric-grid">
        {detail.metrics.map((metric) => <MetricCard key={metric.key} metric={metric} currency={currency} onOpen={() => onOpen(metric)} />)}
      </div>
      <div className="oi-analytics-grid">
        <section className="oi-chart-panel oi-trend-panel">
          <header><div><span>CREATION TREND</span><h4>{detail.label} over time</h4></div><b>{integer(detail.trend.reduce((sum, row) => sum + row.value, 0))} in period</b></header>
          <div className="oi-chart">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={detail.trend} margin={{ top: 12, right: 10, left: -18, bottom: 0 }}>
                <defs><linearGradient id={`oi-trend-${detail.objectType}`} x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--workspace-accent)" stopOpacity={0.28} /><stop offset="95%" stopColor="var(--workspace-accent)" stopOpacity={0.02} /></linearGradient></defs>
        ...
        {detail.breakdowns.map((breakdown) => (
          <section className="oi-chart-panel" key={breakdown.key}>
            <header><div><span>BREAKDOWN</span><h4>{breakdown.label}</h4></div><b>{breakdown.rows.length} groups</b></header>
            <div className="oi-chart">
  ...
  return (
    <div className="oi-drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <aside className="oi-drawer" aria-label={`${title} records`}>
        <header>
          <div><span>LIVE HUBSPOT RECORDS</span><h2>{title}</h2><p>Every row opens the original record in HubSpot.</p></div>
          <button type="button" onClick={onClose} aria-label="Close records"><X size={18} /></button>
        </header>
        <div className="oi-drawer-search"><Search size={15} /><input value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Search this page…" /></div>
        <div className="oi-drawer-body">
          ...
          {!loading && !error && drilldown ? rows.map((row) => {
            const record = recordLabel(drilldown.objectType, row);
            const p = row.properties || {};
            return (
              <article key={row.id}>
                <span className="oi-record-primary">
                  <strong>{record.title}</strong><small>{record.subtitle}</small>
                  {portalId ? <a href={hubSpotRecordUrl(portalId, drilldown.objectType, row.id)} target="_blank" rel="noreferrer">Open in HubSpot <ExternalLink size={12} /></a> : null}
                </span>
...
export function ObjectIntelligenceWorkspace() {
  ...
  const [filters, setFilters] = useState<DateRange>(initialRange);
  const [draft, setDraft] = useState<DateRange>(initialRange);
  const [selectedObject, setSelectedObject] = useState<ObjectType>('contacts');
  const [overview, setOverview] = useState<ObjectOverviewRow[] | null>(null);
  ...
  useEffect(() => {
    ...
    fetchJson<{ results?: WorkspaceRow[] }>('/api/customer/workspaces', { cache: 'no-store', signal: AbortSignal.timeout(20_000) })
      ...
      .catch((error) => active && setOverviewError(error instanceof Error ? error.message : 'Unable to load workspaces.'));
    return () => { active = false; };
  }, []);

  useEffect(() => {
    const captureWorkspaceChange = (event: Event) => {
  ...
  const loadOverview = useCallback(async (nextWorkspaceId: string, nextFilters: DateRange) => {
    ...
    setOverviewError('');
    try {
      const payload = await fetchJson<{ report: { generatedAt: string; objects: ObjectOverviewRow[] } }>(
        `/api/dashboard/${encodeURIComponent(nextWorkspaceId)}/objects?${queryString(nextFilters)}`,
        { cache: 'no-store', signal: controller.signal }
      );
      setOverview(payload.report.objects);
      setGeneratedAt(payload.report.generatedAt);
    } catch (error) {
      if (!controller.signal.aborted) setOverviewError(error instanceof Error ? error.message : 'Object summaries are unavailable.');
    } finally {
  ...
  const loadDetail = useCallback(async (nextWorkspaceId: string, objectType: ObjectType, nextFilters: DateRange) => {
    ...
    setDetailError('');
    try {
      const payload = await fetchJson<{ report: ObjectDetail }>(
      ...
      if (!controller.signal.aborted) setDetailError(error instanceof Error ? error.message : `${objectType} reports are unavailable.`);
    } finally {
  ...
  useEffect(() => {
    if (workspaceId) void loadOverview(workspaceId, filters);
  ...
  const objectRows = useMemo<ObjectOverviewRow[]>(() => overview ?? OBJECT_TYPES.map((objectType) => ({
    objectType,
  ...
  async function openDrilldown(metric: ObjectMetric, offset = 0) {
    ...
    setDrillError('');
    setDrillSearch('');
    if (offset === 0) setDrilldown({ key: metric.key, objectType: selectedObject, columns: [], limit: 50, offset: 0, hasMore: false, results: [] });
    try {
      ...
      if (!controller.signal.aborted) setDrillError(error instanceof Error ? error.message : 'Object records are unavailable.');
    } finally {
  ...
  function exportSnapshot() {
    ...
    const rows: string[][] = [
      ...
      [],
      ...detail.breakdowns.flatMap((breakdown) => [[breakdown.label, 'Count'], ...breakdown.rows.map((row) => [row.key, String(row.value)]), []])
    ];
  ...
  return createPortal(
    <div className="oi-shell">
      <header className="oi-header">
        <div><span><BarChart3 size={14} />OBJECT INTELLIGENCE</span><h2>Every HubSpot object, organized in one reporting workspace.</h2><p>Explore completeness, workload, pipeline, activities and service records without building reports manually.</p></div>
      ...
      <section className="oi-object-grid" aria-label="HubSpot object reports">
        {objectRows.map((row) => <ObjectCard key={row.objectType} row={row} active={selectedObject === row.objectType} onSelect={() => setSelectedObject(row.objectType)} />)}
      </section>
      <section className="oi-detail"><ObjectDashboard detail={detail} loading={detailLoading} error={detailError} currency={currency} onOpen={(metric) => void openDrilldown(metric)} /></section>
      <footer className="oi-footer">
        <span><Activity size={14} />Progressive object reports</span>
        <span>{generatedAt ? `Updated ${new Date(generatedAt).toLocaleString()}` : overviewLoading ? 'Building summaries…' : 'Waiting for workspace data'}</span>
