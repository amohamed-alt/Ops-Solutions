...
async function forward(request: NextRequest, workspaceId: string, init: RequestInit = {}) {
  try {
    const response = await fetch(`${API_URL}/api/v1/customer/workspaces/${encodeURIComponent(workspaceId)}/goals`, {
      ...init,
      headers: { ...customerHeaders(request), ...(init.body ? { 'content-type': 'application/json' } : {}) },
      cache: 'no-store',
      signal: AbortSignal.timeout(15_000)
    });
    const payload = await response.json().catch(() => ({ error: 'invalid_api_response', message: 'The goals service returned an invalid response.' }));
    return NextResponse.json(payload, { status: response.status, headers: { 'cache-control': 'no-store' } });
  } catch (error) {
    return NextResponse.json({
      error: 'goals_service_unavailable',
      message: error instanceof Error ? error.message : 'Unable to reach the goals service.'
    }, { status: 503 });
  }
}
