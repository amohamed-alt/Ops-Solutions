...
export async function GET(
  ...
  try {
    ...
    const target = new URL(
      `${API_URL}/api/v1/workspaces/${encodeURIComponent(workspaceId)}/analytics/extended-objects/${encodeURIComponent(objectType)}`
    );
    ...
    return NextResponse.json({
      error: timedOut ? 'extended_object_detail_timeout' : 'extended_object_detail_unavailable',
      message: timedOut
        ? 'This CRM object report is large. Retry with a shorter date range.'
        : error instanceof Error ? error.message : 'CRM object details are unavailable.'
