...
export default function OperationsClient({ initialWorkspaces }) {
  const [workspaces, setWorkspaces] = useState(initialWorkspaces);
  const [selectedId, setSelectedId] = useState(initialWorkspaces[0]?.workspace?.id ?? '');
  ...
  async function refreshWorkspace(workspaceId, silent = false) {
    const response = await fetch(`/api/operations/${workspaceId}`, { cache: 'no-store' });
  ...
  useEffect(() => {
    if (!selected?.workspace?.id) return undefined;
    const timer = setInterval(() => {
      void refreshWorkspace(selected.workspace.id, true).catch(() => undefined);
    }, selected.activeRun ? 5000 : 30000);
    return () => clearInterval(timer);
  }, [selected?.workspace?.id, selected?.activeRun]);

  if (!selected) {
    return <section className={styles.emptyState}>No workspaces are available yet.</section>;
  }
    ...
    <div className={styles.dashboardGrid}>
      <aside className={styles.workspaceRail}>
        ...
        <div className={styles.workspaceList}>
          {workspaces.map((item) => (
            <button
              className={`${styles.workspaceButton} ${item.workspace?.id === selected.workspace.id ? styles.workspaceButtonActive : ''}`}
              key={item.workspace?.id}
              onClick={() => setSelectedId(item.workspace.id)}
              type="button"
            >
      ...
      <section className={styles.operationsPanel}>
        <header className={styles.operationsHeader}>
          <div>
            <span className="eyebrow">SYNC CONTROL CENTER</span>
            <h1>{selected.workspace.name}</h1>
            <p>Monitor data freshness, object coverage and recovery operations without touching the server.</p>
          </div>
          <div className={`${styles.healthBadge} ${styles[freshness.tone]}`}>
            <span />
            {freshness.label}
          </div>
        </header>
        ...
        <div className={styles.contentGrid}>
          <section className={styles.tableCard}>
            <div className={styles.cardHeader}>
              <div><span className="section-label">DATA COVERAGE</span><h2>CRM objects</h2></div>
              <button type="button" onClick={() => void refreshWorkspace(selected.workspace.id)} disabled={isPending}>Refresh</button>
            </div>
            <div className={styles.objectTable}>
              {(selected.recordCounts ?? []).map((row) => {
                ...
                return (
                  <article key={row.object_type}>
                    <div><strong>{row.object_type}</strong><small>Last success {formatDate(cursor?.last_success_at)}</small></div>
                    <div><span>{formatNumber(row.count)}</span><small>{formatNumber(row.archived_count)} archived</small></div>
                  </article>
                );
              })}
          ...
          <section className={styles.actionsCard}>
            ...
            <p>Operations are deduplicated and blocked while another run is active.</p>
            <div className={styles.actionList}>
              {MODES.map((mode) => (
                <button key={mode.key} type="button" onClick={() => runSync(mode.key)} disabled={isPending || Boolean(selected.activeRun)}>
        ...
        <section className={styles.runCard}>
          <div className={styles.cardHeader}>
            <div><span className="section-label">LATEST RUN</span><h2>Execution summary</h2></div>
            <span className="pill">{selected.latestRun?.mode ?? 'none'}</span>
          </div>
          <div className={styles.runSummary}>
            <div><span>Completed objects</span><strong>{completed.length}</strong></div>
            <div><span>Failed objects</span><strong>{failed.length}</strong></div>
            <div><span>Started</span><strong>{formatDate(selected.latestRun?.started_at)}</strong></div>
            <div><span>Completed</span><strong>{formatDate(selected.latestRun?.completed_at)}</strong></div>
          </div>
