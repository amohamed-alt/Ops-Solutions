...
test('accessible pipeline action opens labelled deals with raw filter values and the correct HubSpot link', async ({ page }) => {
  ...
  const actions = page.locator('.dashboard-pipeline-stage-actions');
  await actions.locator('summary').click();
  await actions.getByRole('button', { name: /Qualified Meeting/i }).click();

  const drawer = page.getByRole('dialog', { name: 'Sales Pipeline · Qualified Meeting' });
  ...
  await expect(drawer.getByText('Synced HubSpot records behind the selected stage.')).toBeVisible();

  const hubspotLink = drawer.getByRole('link', { name: 'Open in HubSpot' });
